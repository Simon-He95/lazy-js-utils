...
export function dragEvent(target: HTMLElement | string, options: DragEvent = {}) {
  ...
  let down = false
  if (isPhone) {
    addEventListener(target as HTMLElement, 'touchstart', (e: any) => {
      options.dragStart && options.dragStart(wrapperE(e))
    }, false)
    options.dragMove && addEventListener(target as HTMLElement, 'touchmove', (e: any) => {
      options.dragMove?.(wrapperE(e))
    }, false)
    options.dragEnd && addEventListener(target as HTMLElement, 'touchend', (e: any) => {
      options.dragEnd?.(wrapperE(e))
      down = false
    }, false)
  }
  else {
    addEventListener(target as HTMLElement, 'mousedown', (e: any) => {
      down = true
      options.dragStart && options.dragStart(e)
    }, false)
    options.dragMove && addEventListener(target as HTMLElement, 'mousemove', (e: any) => {
      if (down)
        options.dragMove?.(e)
    }, false)
    options.dragEnd && addEventListener(target as HTMLElement, 'mouseup', (e: any) => {
      options.dragEnd?.(e)
      down = false
    }, false)
  }
